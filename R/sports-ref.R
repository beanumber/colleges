...
get_bball_year <- function(year) {
  ...
  bball_data %>%
    ...
    mutate(Year = paste0(year - 1, "-", year),
           ...
           bb_champs = grepl(School, pattern = "F+N+$"),
           bb_final_four = grepl(School, pattern = "F+N?$"),
           tourney = grepl(School, pattern = "\\*"),
           conf_champ = grepl(School, pattern = "R+T?F?N?$"),
           conf_tourney = grepl(School, pattern = "\\*.+R?T+F?N?$"),
           school_name = gsub(pattern = "\\*.+R?T?F?N?$", "", School),
           school_name = gsub(pattern = "\\*$", "", school_name),
           school_name = gsub(pattern = ".(R|T)$", "", school_name))
}
